...
export const ProblemController = {
    ...
    async findProblemByDifficulty(req: Request, res: Response, next: NextFunction): Promise<void> {
        const problems = await problemService.findProblemByDifficulty(req.params.difficulty as 'Easy' | 'Medium' | 'Hard');
        res.status(200).json({
            message: "Problems fetched successfully",
            data: problems,
            success: true
    ...
    async searchProblems(req: Request, res: Response, next: NextFunction): Promise<void> {
        const problems = await problemService.searchProblems(req.query.title as string);
        res.status(200).json({
            message: "Problems fetched successfully",
            data: problems,
            success: true
